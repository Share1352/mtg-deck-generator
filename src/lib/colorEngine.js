...
export function chooseDeckColors(cards, { rng = Math.random, logger = null } = {}) {
  ...
  if (!colors.length) colors = sample(COLORS, 2, rng).sort();
...
export function maybeExpandColors({ chosenColors, allCards, needed = 23, logger = null }) {
  const available = allCards.filter((c) => colorIdentityWithin(c, chosenColors));
  if (available.length >= needed || chosenColors.length >= 3) return { colors: chosenColors, expanded: false, available };
  const counts = countColors(allCards);
  const extras = COLORS.filter((c) => !chosenColors.includes(c)).sort((a, b) => counts[b] - counts[a]);
  const expanded = [...chosenColors, extras[0]].filter(Boolean).sort();
  logger?.line(`Color expansion triggered: original=${chosenColors.join('')} valid=${available.length} needed=${needed} expanded=${expanded.join('')}`);
  return { colors: expanded, expanded: true, available: allCards.filter((c) => colorIdentityWithin(c, expanded)) };
}
